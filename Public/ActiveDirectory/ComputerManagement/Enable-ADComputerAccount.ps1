function Enable-ADComputerAccount {
    ...
    try {
        ...
        if ($Computer.Enabled) {
            ...
            Write-Host "`nComputer account is already enabled." `
                -ForegroundColor Yellow

            return
        }

        $Confirmation = Read-Host "`nEnable this computer account? [Y/N]"
        ...
        Write-Host "`nComputer account '$($Computer.Name)' has been enabled successfully." `
            -ForegroundColor Green
    }
    catch {

        Write-Host "`nFailed to enable computer account." `
            -ForegroundColor Red
