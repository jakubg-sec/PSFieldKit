    ...
    try {
        ...
        if ($Context.IsMultiTarget) {
            foreach ($Target in $Context.Targets) {
                Write-Host "`nRetrieving System events from $($Target.ComputerName)..." `
        ...
        if ($null -eq $Events -or $Events.Count -eq 0) {
            Write-Host "`nNo System events found." -ForegroundColor Yellow
            return
        }

        Write-Host "`nSystem Events" -ForegroundColor Cyan
        ...
        Write-Host "`nFailed to retrieve System events." -ForegroundColor Red
        Write-Host $_.Exception.Message -ForegroundColor Yellow
    }
}