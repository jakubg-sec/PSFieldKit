    ...
    try {
        ...
        if ($Context.IsMultiTarget) {
            foreach ($Target in $Context.Targets) {
                Write-Host "`nRetrieving Application events from $($Target.ComputerName)..." `
        ...
        if ($null -eq $Events -or $Events.Count -eq 0) {
            Write-Host "`nNo Application events found." -ForegroundColor Yellow
            return
        }

        Write-Host "`nApplication Events" -ForegroundColor Cyan
        ...
        Write-Host "`nFailed to retrieve Application events." -ForegroundColor Red
        Write-Host $_.Exception.Message -ForegroundColor Yellow
    }
}