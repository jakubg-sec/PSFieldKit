function Set-PSFieldKitExchangeDatabaseRecoverableItemsQuota {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    $CurrentQuota = [string]$Database.RecoverableItemsQuota
    $WarningQuota = [string]$Database.RecoverableItemsWarningQuota

    $CurrentQuotaIsUnlimited = $Database.RecoverableItemsQuota.IsUnlimited
    $WarningQuotaIsUnlimited = $Database.RecoverableItemsWarningQuota.IsUnlimited

    $CurrentQuotaBytes = $null
    $WarningQuotaBytes = $null

    if (-not $CurrentQuotaIsUnlimited) {
        $CurrentQuotaBytes = $Database.RecoverableItemsQuota.Value.ToBytes()
    }

    if (-not $WarningQuotaIsUnlimited) {
        $WarningQuotaBytes = $Database.RecoverableItemsWarningQuota.Value.ToBytes()
    }

    Clear-Host

    Write-Host "+----------------------------------------------+" -ForegroundColor DarkCyan
    Write-Host "|             Recoverable Items Quota          |" -ForegroundColor Cyan
    ...
    Write-Host ("Recoverable Items Warning : {0}" -f $WarningQuota)
    Write-Host ("Recoverable Items Quota   : {0}" -f $CurrentQuota)

    Write-Host "`nNEW RECOVERABLE ITEMS QUOTA" -ForegroundColor DarkCyan
    Write-Host "Examples: 30GB, 30 GB, 50GB, 100GB, 1TB, unlimited"
    ...
    if ($QuotaInput -ieq 'unlimited') {
        ...
        if ($QuotaInput -notmatch '^\s*\d+(?:[.,]\d+)?\s*(B|KB|MB|GB|TB)\s*$') {
            ...
            Write-Host "Use values such as 30GB, 30 GB, 50GB, 100GB, 1TB or unlimited." -ForegroundColor Yellow
            Read-Host "Press Enter to continue" | Out-Null
            return
        }
    ...
    if ($NewQuotaIsUnlimited) {
        ...
        if ($WarningQuotaIsUnlimited) {
            Write-Host "`nRecoverable Items Quota cannot be finite while Recoverable Items Warning Quota is unlimited." -ForegroundColor Red
            Read-Host "Press Enter to continue" | Out-Null
            return
        }

        if ($NewQuotaBytes -lt $WarningQuotaBytes) {
            Write-Host "`nRecoverable Items Quota cannot be lower than Recoverable Items Warning Quota." -ForegroundColor Red
            Write-Host ("Minimum allowed: {0}" -f $WarningQuota) -ForegroundColor Yellow
            Read-Host "Press Enter to continue" | Out-Null
            return
        }
    }
    ...
    if (
        ...
        Write-Host "`nThe database already has Recoverable Items Quota set to '$CurrentQuota'." -ForegroundColor Yellow
        Read-Host "Press Enter to continue" | Out-Null
        return
    }
    ...
    Write-Host ("Warning : {0}" -f $WarningQuota)
    ...
    Write-Host ""
    $Confirmation = Read-Host "Set Recoverable Items Quota to '$NewQuota'? [Y/N]"
    ...
    try {
        Write-Host "`nUpdating Recoverable Items Quota..." -ForegroundColor Yellow

        $Parameters = @{
            Identity              = $Database.Identity
            RecoverableItemsQuota = $NewQuota
            Confirm               = $false
            ErrorAction           = 'Stop'
        }
        ...
        Write-Host "`nRecoverable Items Quota updated successfully." -ForegroundColor Green
    }
    catch {
        Write-Host "`nFailed to update Recoverable Items Quota." -ForegroundColor Red
        Write-Host $_.Exception.Message -ForegroundColor Yellow
        Read-Host "Press Enter to continue" | Out-Null
        return
    }
    ...
    try {
        ...
        $VerificationIsUnlimited = $Verification.RecoverableItemsQuota.IsUnlimited
        ...
        elseif (-not $VerificationIsUnlimited) {
            $VerificationBytes = $Verification.RecoverableItemsQuota.Value.ToBytes()
            $VerificationMatches = $VerificationBytes -eq $NewQuotaBytes
        }

        if ($VerificationMatches) {
            Write-Host "`nRecoverable Items Quota was updated successfully." -ForegroundColor Green
        }
        else {
        ...
        Write-Host "`nRecoverable Items Quota was changed, but verification failed." -ForegroundColor Yellow
