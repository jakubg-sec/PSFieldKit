function Disable-PSFieldKitExchangeSharedMailboxEmailAddressPolicy {
    [CmdletBinding()]
    param(
    ...
    if (-not $Mailbox.EmailAddressPolicyEnabled) {
        Write-Host "Email Address Policy is already disabled for this mailbox." -ForegroundColor Yellow
        Read-Host "`nPress Enter to continue" | Out-Null
        return
    }

    Write-Host "Disabling Email Address Policy will stop automatic email address" -ForegroundColor Yellow
    Write-Host "policy management for this mailbox." -ForegroundColor Yellow
    Write-Host ""
    Write-Host "The mailbox will keep its current email addresses unless they are" -ForegroundColor Yellow
    Write-Host "changed manually or by another process." -ForegroundColor Yellow
    Write-Host ""

    $Confirmation = Read-Host "Type DISABLE POLICY to continue"
    ...
    try {
        ...
        Write-Host ""
        Write-Host "Email Address Policy disabled successfully." -ForegroundColor Green
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "Failed to disable Email Address Policy." -ForegroundColor Red
        Write-Host $_.Exception.Message -ForegroundColor Yellow
    }
